<template>
  <div class="game-view">
    <div class="game-header">
      <h1 class="game-title text-chinese">句子排序</h1>
      <p class="game-subtitle">{{ t('view.sentence') }}</p>
    </div>
    ...
    />
    <GameOverModal
      v-if="gameState.isComplete.value && !infiniteMode" :title="t('game.excellent')"
...
import SentenceBoard from '../components/sentence/SentenceBoard.vue'
...
function onGameComplete() {
  ...
  addGameSession('sentences', timer.elapsed.value, score.correct.value)
}
function onRoundComplete() {
  if (infiniteMode.value) addGameSession('sentences', timer.elapsed.value, score.correct.value)
}
