...
import WordSorting from '../components/sorting/WordSorting.vue'
...
function onCorrect(payload) { score.addCorrect(10); if (payload?.word) recordWordCorrect(payload.word) }
function onIncorrect() { score.addIncorrect() }
function onGameComplete() {
  if (infiniteMode.value) return
  timer.pause(); gameState.complete()
  addGameSession('sorting', timer.elapsed.value, score.correct.value)
}
function onRoundComplete() {
  if (infiniteMode.value) addGameSession('sorting', timer.elapsed.value, score.correct.value)
}
